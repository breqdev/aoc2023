...
with open("input.txt") as f:
    for line in f.readlines():
        ...
        groups = tuple(int(i) for i in groups_line.split(","))
        lines.append((springs_line, groups))


def unfold(line: str, groups: tuple[int, ...]):
    line_result = "?".join(line for _ in range(5))
    groups_result = groups * 5
    return line_result, groups_result


lines = [unfold(line, groups) for line, groups in lines]


@functools.cache
def solve_line(
    line: str, groups: tuple[int], remaining_in_group: int = 0, in_group=False
) -> int:
    if len(line) == 0:
        if len(groups) > 0:
            return 0
